...
class BaseManager(ABC):
    ...
    def __init__(self, service_name: str):
        ...
        logger.debug(
            f"{self.service_name} manager initialized", extra={"interval": self.get_schedule_info()}
        )
    ...
    def run(self) -> None:
        ...
        """
        logger.debug(f"Running {self.service_name} cycle")
        try:
            self._execute_cycle()
            logger.debug(f"{self.service_name} cycle completed")
        except Exception as e:
            logger.error(
                f"Error during {self.service_name} cycle",
